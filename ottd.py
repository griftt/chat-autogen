...
#main
tab1, tab2,cloth = st.tabs(["📈 私人管家", "🗃 AI团队","换装"])
...
cloth.subheader("模特换装")
# 设置页面标题
cloth.title('图片上传示例')

# 创建两列布局，用于显示模特图片和服装图片


col1, col2, col3 = st.columns(3)


# 在第一列显示模特图片上传组件和上传后的图片
with col1:
    model_file = st.file_uploader("上传模特图片", type=["jpg", "jpeg", "png"])
    if model_file is not None:
        model = Image.open(model_file)
        st.image(model, caption='模特', use_column_width=True)

# 在第二列显示服装图片上传组件和上传后的图片
with col2:
    dress_file = st.file_uploader("上传服装图片", type=["jpg", "jpeg", "png"])
    if dress_file is not None:
        dress = Image.open(dress_file)
        st.image(dress, caption='服装图片', use_column_width=True)