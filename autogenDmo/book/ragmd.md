...
在这个例子中，问题是直接从数据集中选择的。RetrieveChat能够在第一次尝试中正确回答问题，因为检索到的上下文包含前两种情况下的必要信息。然而，在最后三种情况下，与问题嵌入相似度最高的上下文并不包含回答问题所需的信息。因此，LLM型号的响应为 UPDATE CONTEXT 。凭借RetrieveChat中更新上下文的独特创新能力，代理自动更新上下文并将其再次发送到LLM模型。经过几轮这样的过程，代理能够生成问题的正确答案。

Example 6 实施例6
Back to top  返回顶部

Use RetrieveChat to answer multi-hop questions for 2WikiMultihopQA dataset with customized prompt and few-shot learning.
使用RetrieveChat通过自定义提示和少量学习回答2WikiMultihopQA数据集的多跳问题。
...
首先，我们将创建一个包含所有上下文语料库的新文档集合。然后，我们将选择一些问题，并利用RetrieveChat来回答他们。对于这个特殊的例子，我们将使用 gpt-3.5-turbo 模型，我们将演示RetrieveChat的自动更新上下文的功能，如果检索到的文档不包含足够的信息。此外，我们将演示如何使用自定义提示和少量学习来解决RetrieveChat中未预定义的任务。
